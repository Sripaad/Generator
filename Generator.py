...
def readInTrainData(kind='male', k=2):
    ...
    # Add '__' to start of name and '**' at end of name (pad)
    nameLst = ['_'*k + name + '*'*k for name in orig_names]
    return nameLst, orig_name
...
def createTransitionMatrix(names, k):
    ...
    for name in names:
        ...
        for i in range(len(letters)):
            # creating the reference sequence of letters for a given n
            tempLst = []
            ...
            if letters[i+k] not in countDict[letterSeq].keys():
                countDict[letterSeq][letters[i+k]
